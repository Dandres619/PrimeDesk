...
function AppContent() {
    ...
    const [currentUser, setCurrentUser] = useState<{ username: string; name: string; type: string; token?: string } | null>(null);
    ...
    const handleLogin = (userData: { username: string; name: string; type: string; token: string }) => {
        setCurrentUser(userData);
        setIsAuthenticated(true);
        setShowLanding(false); // Ocultar landing al hacer login
        // Aquí se podría guardar el token en localStorage si se desea persistencia
    ...
    const confirmLogout = () => {
        ...
        window.location.reload(); // Forzar recarga para limpiar memoria y estado
    };
