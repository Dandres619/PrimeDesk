...
import { Wrench, Search, MessageSquare, AlertCircle, Info } from 'lucide-react';
...
export function AptServicesFields({
  ...
  selectedMechanicSchedule, selectedMechanic
}: any) {
  return (
    <>
      {!apt && (
              ...
              selectedMechanicSchedule && durationData.endTime > selectedMechanicSchedule.salida ? (
                <div className="bg-rose-50 dark:bg-rose-950/40 border border-rose-100 dark:border-rose-900/40 px-4 py-2.5 rounded-xl text-left flex items-center gap-2.5 animate-fadeIn">
                  ...
                  <span className="text-xs font-bold text-rose-600 dark:text-rose-400">
                    La hora de finalización excede la hora de salida del mecánico {selectedMechanic?.Nombre} {selectedMechanic?.Apellido} ({format12h(selectedMechanicSchedule.salida)}). Elija otro mecánico o reduzca los servicios.
                  </span>
                </div>
              ) : (
