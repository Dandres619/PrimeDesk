...
const update = async (id, { id_cliente, marca, modelo, anio, placa, color, motor, kilometraje, estado }) => {
  ...
  // Si intentan inactivar, verificar dependencias primero
  if (estado === false) {
    const agendamientos = await sql`SELECT 1 FROM agendamientos a INNER JOIN reparaciones r ON r.id_agendamiento = a.id_agendamiento WHERE r.id_motocicleta = ${id} LIMIT 1`;
